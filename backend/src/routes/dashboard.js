...
router.get('/', async (req, res) => {
  const [
    openRequirements, recruiterReview, withBde, clientReview, interviewsScheduled, hired, offers, auditLog,
  ] = await Promise.all([
    ...
    prisma.application.count({ where: { stage: { in: ['OFFER', 'OFFER_ACCEPTED'] } } }),
