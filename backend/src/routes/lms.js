...
router.post('/courses', requireRole(...HR_ROLES), async (req, res) => {
...
router.post('/assignments', requireRole(...HR_ROLES), async (req, res) => {
...
router.patch('/assignments/:id/complete', async (req, res) => {
  ...
  if (req.user.role === 'EMPLOYEE' && assignment.employee.userId !== req.user.id) {
