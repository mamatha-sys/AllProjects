...
export default function Candidates() {
  ...
  const [form, setForm] = useState({ name: '', email: '', phone: '', source: 'TeamLink Website', skills: '' });
  ...
  async function checkDuplicate() {
    if (!form.email && !form.phone) return setDupWarning('');
    const res = await api.get('/candidates/check-duplicate', { params: { email: form.email, phone: form.phone } });
    setDupWarning(res.data.duplicate ? `Possible duplicate: ${res.data.matches.map((m) => m.name).join(', ')} already exists.` : '');
  }

  async function createCandidate(e) {
    e.preventDefault();
    await api.post('/candidates', form);
    setForm({ name: '', email: '', phone: '', source: 'TeamLink Website', skills: '' });
    setDupWarning('');
    setShowForm(false);
    load();
  }
        ...
        <form className="card section" onSubmit={createCandidate}>
          <div className="grid-2">
            ...
            <label className="field">
              ...
              <input value={form.skills} onChange={(e) => setForm({ ...form, skills: e.target.value })} placeholder="Node.js, IT, Backend" />
            </label>
          </div>
          {dupWarning && <div className="error-text">{dupWarning}</div>}
